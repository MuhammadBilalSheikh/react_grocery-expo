...
const databaseUrl = "postgresql://neondb_owner:npg_rkDs02zxecZC@ep-nameless-snow-anag3fc6-pooler.c-6.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require";
    ...
    for (const item of seedItems) {
        await sql`
            INSERT INTO grocery_items (id, name, category, quantity, purchased, priority, updated_at)
                VALUES (${crypto.randomUUID()}, ${item.name}, ${item.category}, ${item.quantity}, ${item.purchased}, ${item.priority}, ${Date.now()})
        `;
    }
